...
var _webrtc = require(45);

var _setup = require(42);

var _VideoStage = require(36);

var _BackgroundWorker = require(34);

var _workerMsg = require(37);

var _animation = require(1);

var _ProcessWindow = require(35);
...
let stage = new _VideoStage.VideoStage();
let worker = new _BackgroundWorker.BackgroundWorker(Flip.$('#worker').value);
let predictReady = false;
let processWindow = new _ProcessWindow.ProcessWindow();
let currentAnimation;
worker.onmessage(_workerMsg.MSG_EVENT_WORKER_READY, function () {
  worker.postMessage(_workerMsg.MSG_EVENT_INIT_NET);
});
worker.onmessage(_workerMsg.MSG_EVENT_INIT_NET, function () {
  predictReady = true;
});
worker.onmessage(_workerMsg.MSG_EVENT_PREDICT, function (d) {
  processWindow.push(d);
  let maxIndex = processWindow.currentIndex;
  if (maxIndex > -1 && !(0, _animation.isTransitioning)() && currentAnimation != animations[maxIndex]) {
    (0, _animation.transitionAnimationAsync)(currentAnimation = animations[maxIndex]);
  }
});
(0, _webrtc.getCameraVideoOfSizeAsync)().then(function (result) {
...
function createCaptureRender(canvas) {
  let ctx = canvas.getContext('2d');
  let render = new Flip.GL.Render({ name: 'capture' });
  render.render = function (e) {
    if (predictReady && !worker.busy && !(0, _animation.isTransitioning)()) {
      let imageData = stage.captureFrame(e.gl, processFrame);
      /* canvas.width = imageData.width;
       canvas.height = imageData.height;
       ctx.putImageData(imageData, 0, 0);*/
      worker.postMessage(_workerMsg.MSG_EVENT_PREDICT, imageData, [imageData.data.buffer]);
    }
    /**/
  };
  return render;
...
exports.BackgroundWorker = undefined;
...
class BackgroundWorker {
  ...
  constructor(src) {
    ...
    worker.onmessage = function (e) {
      let msg = e.data;
      let type = msg.type;
      if (type == _workerMsg.MSG_EVENT_WORKER_READY) {
        self.ready = true;
      }
...
exports.ProcessWindow = ProcessWindow;
...
var _OpeningFilter = require(30);

var _SobelEdgeFilter = require(31);

var _GLFilter = require(28);

var _MirrorFilter = require(29);
